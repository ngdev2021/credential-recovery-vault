...
export function rotateRecoveryCodes(codes: RecoveryCode[], rotatedAt = new Date().toISOString()): RecoveryCode[] {
  return codes.map((code) => (
    code.status === 'unused' || code.status === 'used'
      ? { ...code, status: 'replaced', rotatedAt }
      : code
  ));
}
